...
| `favicon.svg` | Tab icon: a calendar with the busyness curve inside. One flat glyph in the shared Clearful icon blue (`#3DADFF`) on a transparent background, matching every sibling app's favicon; not a token colour on purpose. |
| `index.html` | Frontend shell. Controls toolbar (outlook switch + view filters, a dropdown on phones), forecast strip, detail/map region, footer with the shared `/images/icon-rw.svg` back-to-apps mark. Loads Leaflet from unpkg. Opens with `data-booting` on `<html>` (nothing paints until tokens, styles, fonts and the first render are in; a 6 s inline fallback clears it if scripts never arrive) and each top-level block carries `.reveal` + `--reveal-i` for the staggered entrance. `tokens.js` loads in `<head>` so the custom properties exist before first paint. |
| `app.js` | Entry point. `bootReveal` clears `data-booting` once the first render and the web font are both in (4 s deadline), then drops the `.reveal` hooks so a later hide/show doesn't replay the entrance. Owns the outlook view switch (7 days / Month), fetches + rehydrates archived days into `state.forecasts` so past days render through the same timeline / map / rail path as live ones. Loads cities, renders the 7-day verdict-pill strip (each pill carries a mini busyness graph on a shared time window and absolute scale), drives day selection, and renders the selected day today-first: "Because …" driver line, then the timeline (an event lane above the curve, the stations likely packed as lanes below it; subway by default, Streetcar / GO toggles), map, events-at-time rail. View filters (Sports / Concerts / Theatre & other / Smaller venues) are client-side; verdicts stay full-model unless the switch is on; all persisted in localStorage. |
...
- **One phone breakpoint: `max-width: 720px`.** Below it the page is edge to edge: `.app-main` drops its gutter, each block carries its own 16px inset, and the detail card loses its side borders and radius. Don't add per-component breakpoints.
- **Touch sizing lives in `(pointer: coarse)` media queries, not in the phone breakpoint.** A chip grows to `--size-control` (32px) and an invisible `::after` halo pads its hit area to `--size-tap` (48px); lane chips, map zoom buttons, calendar arrows and Tickets links follow the same two tokens. Fine pointers keep the compact sizes. Hover styles sit under `@media (hover: hover)` so they can't stick to the last thing tapped.
- **Dropdowns, phones only.** The view filters open as a dropdown from the `Filters` chip at the right end of the toolbar; the timeline's Stations and Jump-to (Peak / Now) chips open from `Options` at the right end of the chip row, while the Range chips (All day / Fit) stay in the row. Both chips carry a count of settings switched on, and both dropdowns close on outside click or Escape. The CSS shows the chips and honours `data-collapsed` only under the phone breakpoint; on wider screens the panels are inline (`display: contents`) and every chip is visible.
- **Nothing traps the page scroll.** The chart and the brush use `touch-action: pan-y` and only take a touch once it moves sideways (`touchIntent` in `timeline.js`); the map has one-finger `dragging` off on coarse pointers (two fingers pan and pinch) and says so in its hint.
- **No hover-only information.** Anything in a `title` also reaches touch and screen readers another way: the lane popover, the legend's (i) detail line, `.sr-only` descriptions.
- **`[hidden]` always wins** (global `display: none !important`), so a component's own `display: flex` can never leave an empty box on screen.
- **Contrast floor is `--color-text-tertiary`** (4.5:1 or better on every dark surface). `--color-text-muted` is for disabled controls and decorative separators only, never for text someone has to read.
