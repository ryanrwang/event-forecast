...
| `timeline.js` | Custom canvas day timeline (smooth monotone curve) + scrubber + per-event In / Out bands (M3), a persisted dual-handle range brush above the chart (default 9 AM → 2 AM; "All day" for the full 26 h), and station lanes under the curve (badge + name chip per busy window, hover for details, click to scrub). Bucket count comes from the day file. |
